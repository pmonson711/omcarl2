module ParserEngine = struct
  ...
  let rec parse lexbuf (checkpoint : Grammar.Spec.t Interpreter.checkpoint) =
    let module I = Interpreter in
  ...
  let parse_from lexbuf =
    let open Lexing in
    try
      let checkpoint = Parser.Incremental.spec lexbuf.lex_curr_p in
...
let parse_from_string str = str |> Lexing.from_string |> ParserEngine.parse_from
